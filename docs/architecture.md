...
├── 手机小组件推文 ◀──────── CC 调用 POST /api/tweets
└── 手机小组件明信片 ◀───── CC 调用 POST /api/mails
```
...
一个基于 Express.js 的网页服务器，提供浏览器端的聊天界面。

- **作用**：让你可以在浏览器里跟 companion 聊天
- **功能**：
  - 密码登录验证
  - 聊天记录存储（SQLite 数据库）
  - 自定义表情包面板（可以发图片/贴纸）
- **与 CC 的连接方式**：通过 `web-channel` MCP 插件，消息会自动出现在 CC 的上下文中
- **端口号**：可在 `.env` 中配置

### Phone Widget（手机小组件）

一个轻量级的 Node.js 服务器，提供手机端的小组件界面。

- **作用**：在手机上展示推文（tweets）、明信片（postcards）、以及轻量聊天
- **技术细节**：
  - 原生 Node.js HTTP + WebSocket 服务器（没有用 Express 框架）
  - 前端服务端口和内部 MCP 桥接端口分开（端口号可在 `.env` 中配置）
  - `bridge.js` 是一个 MCP stdio 服务器，CC 通过它来通信
- **数据存储**：推文和明信片分别存在 JSONL 文件里
...
| 数据类型 | 存储方式 | 位置 |
|----------|----------|------|
| 聊天记录 | SQLite 数据库 | `chat-server/` 目录下 |
