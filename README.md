...
- **CC → API**：在设置面板里开启「CC Sync」开关后，切换到 API 模式时，当前对话窗口里的 CC 聊天记录会被自动整理（较早的压缩为摘要，最近几轮保留原文），注入到 API 请求中。API 模型能看到你和 CC 刚才聊了什么。
- **API → CC**：切回 CC 模式时，系统会自动检测模式切换，把最近的 API 对话内容带给 CC，让它接上之前的话题。

上下文互通是按对话窗口隔离的 —— 只拉当前窗口内的消息，换一个新窗口就是干净的开始。
